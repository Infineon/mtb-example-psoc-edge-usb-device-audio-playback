...
The following example clones the "[Audio Playback using emUSB-device](https://github.com/Infineon/mtb-example-psoc-edge-usb-device-audio-playback)" application with the desired name "AudioPlayback" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-usb-device-audio-playback --user-app-name AudioPlayback --target-dir "C:/mtb_projects"
   ```
